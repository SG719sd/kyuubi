...
export default function PiattoDrawer({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. Elaborazione e Upload Immagine via Server Action
    if (imageFile && piattoId) {
      try {
        ...
        if (!uploadRes.success || !uploadRes.url) {
          throw new Error(uploadRes.error || "Errore caricamento immagine");
        }
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex sm:pl-10">
        <div className="w-full sm:w-screen max-w-full sm:max-w-xl bg-white dark:bg-slate-900 shadow-2xl flex flex-col justify-between border-l border-slate-200 dark:border-slate-800">
          {/* Header */}
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/50">
            <div>
              <h2 className="font-bold text-slate-900 dark:text-white text-base tracking-tight flex items-center gap-2">
                <UtensilsCrossed className="w-4 h-4 text-indigo-600 dark:text-indigo-400 shrink-0" />
