-- ==============================================================================
-- KYUUBI / EAZYHUBS - DATABASE SCHEMA DEFINITION (POSTGRESQL / SUPABASE)
-- Includes: DDL, Indexes, Checks, Triggers, RLS Policies, Helper Functions
-- ==============================================================================
...
-- Trigger to auto-create public.users row on auth.users signup
...
create trigger trg_hub_billing_info_updated_at
    before update on public.hub_billing_info
    for each row execute function public.handle_updated_at();

-- ==============================================================================
-- 6. PROFESSIONISTI (STAFF & PERMISSION MAPPING)
-- ==============================================================================
...
create trigger trg_professionisti_updated_at
    before update on public.professionisti
    for each row execute function public.handle_updated_at();

-- Trigger: Automatically make the creator of an Hub an Admin Professionista in that Hub
...
begin
    ...
    insert into public.professionisti (
        ...
        new.id,
        new.id_user,
        coalesce(trim(v_user_nome || ' ' || v_user_cognome), 'Amministratore'),
...
-- ==============================================================================
-- 7. CONSENSI & AUTORIZZAZIONI (Digital Signature & Audit Trail)
-- ==============================================================================
...
-- ==============================================================================
-- 9. PRODOTTI (MAGAZZINO / INVENTARIO)
-- ==============================================================================
...
-- ==============================================================================
-- 10. PIATTI (MENU DIGITALE / RISTORAZIONE)
-- ==============================================================================
...
create trigger trg_piatti_updated_at
    before update on public.piatti
    for each row execute function public.handle_updated_at();

-- ==============================================================================
-- 11. RUBRICA (CLIENTI & ANAGRAFICHE HUB)
-- ==============================================================================
create table if not exists public.rubrica (
    ...
    constraint fk_rubrica_user foreign key (id_user) references public.users (id) on delete set null,
...
-- ==============================================================================
-- 12. PROFESSIONISTI - SERVIZI (CUSTOM PRICE & DURATION MAPPING)
-- ==============================================================================
...
create trigger trg_prof_servizi_updated_at
    before update on public.professionisti_servizi
    for each row execute function public.handle_updated_at();

-- ==============================================================================
-- 13. ORARI LAVORO (HUB & PROFESSIONISTI SCHEDULES)
-- ==============================================================================
...
create trigger trg_orari_lavoro_updated_at
...
-- ==============================================================================
-- 16. SECURITY HELPER FUNCTIONS (FOR RLS)
-- ==============================================================================
-- Function: Check if auth.uid() is an Admin or Owner for a specific Hub
...
-- Function: Check if auth.uid() is an Active Member (any role) in a specific Hub
...
-- HUBS POLICIES
-- 1. Management: Hub members can view the hub
create policy "Hub members can select hub" on public.hubs
    for select using (public.is_hub_member(id));

-- 2. Public Showcase: EazyHubs public view for active & visible hubs
...
-- PROFESSIONISTI SERVIZI POLICIES
